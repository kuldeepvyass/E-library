  ...
  <body>
    <form name="form1" method="post" action="">
      ...
      <div class="nav">
        <ul>
          <li><a style="background-color: orange" href="student.php">Home</a></li>
      ...
      <?php while ($row = mysqli_fetch_array($results)) { ?>
        ...
        <div class="main">
          <?php if (isset($_POST['searchbtn'])) {
            $query = "SELECT * FROM books_record INNER JOIN user_record ON books_record.Sem = user_record.Semester AND User_id='$_SESSION[uid]' AND user_record.Department=books_record.Course WHERE Book_name LIKE('%$_POST[search]%') OR AuthOR LIKE('%$_POST[search]%') OR Publication LIKE('%$_POST[search]%') OR Category LIKE('%$_POST[search]%')";
          ...
          ?>
            <table>
              ...
              <?php
              $query = "SELECT * FROM books_record INNER JOIN user_record ON books_record.Sem = user_record.Semester AND User_id='$_SESSION[uid]' AND user_record.Department=books_record.Course";
              $results = mysqli_query($conn, $query);
